const destinations = [
  ...
  const createDestinationCard = (destination) => {
    ...
    card.innerHTML = `
          ...
          <div class="destination-card-content">
              ...
              <button class="btn favorite-btn" data-id="${destination.id}">
  ...
  const handleFavoriteClick = (event) => {
    if (event.target.classList.contains("favorite-btn")) {
      ...
      toggleFavorite(id)
      event.target.textContent = isFavorite(id) ? "Remove from Favorites" : "Add to Favorites"
    }
  }
  
  document.addEventListener("DOMContentLoaded", () => {
    renderDestinations()
    document.getElementById("destinations-grid").addEventListener("click", handleFavoriteClick)
  })
